...
const jdynbApi = "http://m.jdynb.xyz/api";
const pageSize = 30;
let page;
...
// 基于 https://github.com/lxmusics/lx-music-api-server 实现的公益音源 + jdynb 镜像兜底
    ...
    try {
        switch (String(err_i)) {
            ...
            case "1": // By: 微信公众号@元力菌
                ...
                break;
            case "4": // By: jdynb (http://m.jdynb.xyz) —— kuwo 镜像站解析兜底
                // 该站 play/info 可直接用酷我 rid 取播放链接（需 API-Key 头）
                let bridge = {
                    "low": "128kmp3",
                    "standard": "128kmp3",
                    "high": "2000kflac",
                ...
                res = (
                    await axios_1.default.get(`${jdynbApi}/music/play/info`, {
                        params: {
                            id: musicItem.id,
                            bridge: bridge
                        },
